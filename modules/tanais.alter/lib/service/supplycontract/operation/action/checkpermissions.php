...
class CheckPermissions extends Operation\Action
{
    ...
    public function process(Item $item): Main\Result
    {
        ...
        // Запрещаем пропуск стадии, кроме разрешённых переходов
        $allowedSkipTransitions = [
            'DT1074_32:UC_SOAZ1F' => [
                'DT1074_32:UC_3FJ09G',
                'DT1074_32:UC_2IO5H6',
            ],
        ...
        $isAllowedSkip = isset($allowedSkipTransitions[$currentStageId])
            && in_array($newStageId, $allowedSkipTransitions[$currentStageId], true);

        if (!str_contains($newStageId, ':FAIL') && ($newStageIndex - $currentStageIndex) >= 2 && !$isAllowedSkip) {
...
//        }
        ...
        if ($allowedUser)
            if (!in_array($userId, $allowedUser)) {
                $errorMessage = 'Изменение стадии с "' . $stagesName[$currentStageId] . '" доступно только: ' . \Tanais\Alter\User::getUserNames($allowedUser) . ' ' . $test . ' ' . $stagesName[$currentStageId] . ".";
                $result->addError(new Error($errorMessage));
            }
