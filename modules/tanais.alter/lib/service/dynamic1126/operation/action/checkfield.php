...
namespace Tanais\Alter\Service\Dynamic1126\Operation\Action;
...
class CheckField extends Operation\Action
{
    ...
    public function process(Item $item): Main\Result
    {
        ...
        $factory = $container->getFactory(\Tanais\Alter\Service\Container::AGR_COMPANYES_ENTITY_ID);

        $code = $item->get('UF_CRM_23_CODE');

        $elements = $factory->getItems([
            'order' => ['TITLE' => 'ASC'],
            'select' => ['ID', 'UF_CRM_23_CODE'],
            'filter' => [
                '!ID' => $item->getId(),
                'UF_CRM_23_CODE' => $code,
            ],
